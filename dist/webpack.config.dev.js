...
var config = require('./config/main'); // Проверка настроек


module.exports = function (env, option) {
  var production = option.mode === 'development' ? false : true;
  var pluginsOptions = [];
  var minimizer = []; // Очистка папки вывода
  ...
  {
    var pages = glob.sync(__dirname + '/' + config.folders.input.pages + '\\**\\*.pug');
    pages.forEach(function (file) {
      var base = path.relative(__dirname + '/' + config.folders.input.pages, file);
  ...
  var webpack_config = {
    entry: {
      main: path.resolve(__dirname, config.folders.input.base + '/' + config.scripts.entry_name + '.js')
    },
    output: {
    ...
    devServer: {
      ...
      watchOptions: {
        poll: true,
        ignored: ['/node_modules/', '/.git/', '/.vscode/', '/config/', path.resolve(__dirname, config.folders.output.base)]
      }
    }
  };
